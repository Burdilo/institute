...
;( function( $){
	...
	$.app.cart = {
	    ...
	    {
			/**
	         * ÐžÐ±Ñ€Ð°Ð±Ð¾Ñ‚Ñ‡Ð¸Ðº ÐºÐ»Ð¸ÐºÐ° Ð½Ð° Ð´Ð¾Ð±Ð°Ð²Ð»ÐµÐ½Ð¸Ðµ Ð² ÐºÐ¾Ñ€Ð·Ð¸Ð½Ñƒ
	         */
	        'click | .js-cart-add': '_buy',

	        /**
			 * ÐžÐ±Ñ€Ð°Ð±Ð¾Ñ‚Ñ‡Ð¸Ðº ÐºÐ»Ð¸ÐºÐ° Ð½Ð° ÑƒÐ´Ð°Ð»ÐµÐ½Ð¸Ðµ Ð¸Ð· ÐºÐ¾Ñ€Ð·Ð¸Ð½Ñ‹
			 */
	        'click | .js-cart-remove': '_remove',

	        /**
			 * ÐžÐ±Ñ€Ð°Ð±Ð¾Ñ‚Ñ‡Ð¸Ðº ÐºÐ»Ð¸ÐºÐ° Ð½Ð° Ð¸Ð·Ð¼ÐµÐ½ÐµÐ½Ð¸Ðµ ÐºÐ¾Ð»Ð¸Ñ‡ÐµÑÑ‚Ð²Ð° Ñ‚Ð¾Ð²Ð°Ñ€Ð° Ð² Ñ‚Ð°Ð±Ð»Ð¸Ñ†Ðµ
			 */
	    ...
	    methods:
	    {
				...
				$.ajax({
		            ...
		            success: function(response)
		            {
		                ...
		                if (response.status === true)
		                {
							...
							{
								$('#catalog-product-' + id).find('.js-incart-holder').html( '<a href="#item-' + id + '" class="product__remove-link js-cart-remove" title="Ð£Ð´Ð°Ð»Ð¸Ñ‚ÑŒ Ð¸Ð· ÐºÐ¾Ñ€Ð·Ð¸Ð½Ñ‹">Ð£Ð´Ð°Ð»Ð¸Ñ‚ÑŒ Ð¸Ð· ÐºÐ¾Ñ€Ð·Ð¸Ð½Ñ‹</a>');
							}
		                }
			  	...
			  	$.ajax({
		            ...
		            success: function(response)
		            {
		            	...
		            	if (response.status === true)
		                {
							...
							{
								$('#cart-table').find('tbody').append('<tr class="cart-table-empty"><td colspan="8">Ð’ ÐºÐ¾Ñ€Ð·Ð¸Ð½Ðµ Ð¿ÑƒÑÑ‚Ð¾.</td></tr>');
							}
							...
							{
								$('#catalog-product-' + id).find('.js-incart-holder').html( '<a href="#item-' + id + '" class="product__buy-link js-cart-add" title="Ð”Ð¾Ð±Ð°Ð²Ð¸Ñ‚ÑŒ Ð² ÐºÐ¾Ñ€Ð·Ð¸Ð½Ñƒ">Ð”Ð¾Ð±Ð°Ð²Ð¸Ñ‚ÑŒ Ð² ÐºÐ¾Ñ€Ð·Ð¸Ð½Ñƒ</a>');
							}
		                }
			...
			_message: function(text)
			{
			    ...
			    if (!body.find('.popup-addcart-message').length)
			    {
					...
					// 		'<a href="#" onclick="return $.cart.continue(event)" class="btn btn-continue">ÐŸÑ€Ð¾Ð´Ð¾Ð»Ð¶Ð¸Ñ‚ÑŒ Ð¿Ð¾ÐºÑƒÐ¿ÐºÐ¸</a>',
					// 		'<a href="/basket" class="btn btn-checkout">ÐžÑ„Ð¾Ñ€Ð¼Ð¸Ñ‚ÑŒ Ð·Ð°ÐºÐ°Ð·</a>',
					// 	'</div>',
					// '</div>'
...
//         /**
//          * ÐžÐ±Ñ€Ð°Ð±Ð¾Ñ‚Ñ‡Ð¸Ðº ÐºÐ»Ð¸ÐºÐ° Ð½Ð° Ð¿Ð»ÑŽÑ
//          */
...
//         /**
//          * ÐžÐ±Ñ€Ð°Ð±Ð¾Ñ‚Ñ‡Ð¸Ðº ÐºÐ»Ð¸ÐºÐ° Ð½Ð° Ð¼Ð¸Ð½ÑƒÑ
//          */
...
//         /**
//          * Ð£Ð²ÐµÐ»Ð¸Ñ‡Ð¸Ñ‚ÑŒ ÑÑ‡ÐµÑ‚Ñ‡Ð¸Ðº
//          */
//         inc: function() {
...
//         /**
//          * Ð£Ð¼ÐµÐ½ÑŒÑˆÐ¸Ñ‚ÑŒ ÑÑ‡ÐµÑ‚Ñ‡Ð¸Ðº
//          */
//         dec: function() {
//             this.count -= this.params.step;
//         },
...
//         /**
//          * ÐÐ°Ñ€Ð¸ÑÑƒÐµÑ‚ Ð½Ð¾Ð²Ð¾Ðµ Ð·Ð½Ð°Ñ‡ÐµÐ½Ð¸Ðµ
//          */
//         update: function() {
		...
		init: function()
		{
			...
			if ($('.js-suggestions').length)
			{
				...
				$('.js-suggestions').each(function() {
					...
					if (suggestion == 'org')
					{
						// ÐŸÐ¾Ð´ÑÐºÐ°Ð·ÐºÐ¸ Ð¿Ð¾ Ð¾Ñ€Ð³Ð°Ð½Ð¸Ð·Ð°Ñ†Ð¸ÑÐ¼ (jQuery)
						$("#party").suggestions({
					        serviceUrl: "https://dadata.ru/api/v2",
					        token: "Ð’ÐÐ¨ API-ÐšÐ›Ð®Ð§",
					        type: "PARTY",
					        /* Ð’Ñ‹Ð·Ñ‹Ð²Ð°ÐµÑ‚ÑÑ, ÐºÐ¾Ð³Ð´Ð° Ð¿Ð¾Ð»ÑŒÐ·Ð¾Ð²Ð°Ñ‚ÐµÐ»ÑŒ Ð²Ñ‹Ð±Ð¸Ñ€Ð°ÐµÑ‚ Ð¾Ð´Ð½Ñƒ Ð¸Ð· Ð¿Ð¾Ð´ÑÐºÐ°Ð·Ð¾Ðº */
					        onSelect: function(suggestion) {
					            console.log(suggestion);
					        }
					    });
					...
					// ÐŸÐ¾Ð´ÑÐºÐ°Ð·ÐºÐ¸ Ð¿Ð¾ Ð¤Ð˜Ðž (jQuery)
			...
			// // Ð³Ð¾Ñ€Ð¾Ð´ Ð¸ Ð½Ð°ÑÐµÐ»ÐµÐ½Ð½Ñ‹Ð¹ Ð¿ÑƒÐ½ÐºÑ‚
			// $city.suggestions({
			// 	serviceUrl: serviceUrl,
			// 	token: token,
			// 	type: type,
